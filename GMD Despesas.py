...
button[kind=secondary]{background-color:#1F3864 !important;color:white !important;border:none !important;border-radius:8px !important;font-weight:600 !important}
button[kind=secondary]:hover{background-color:#2E75B6 !important}
.stFileUploader label{color:#1F3864 !important;font-weight:600 !important}
...
def resumo(merged, mc, mes, ano):
    ...
    if 'Descricao da conta' not in grp.columns:
        grp = merged.groupby(['Nome do pacote','Estr. da conta','Descrição da conta','Gestor do pacote'], as_index=False)[mc].sum()
        grp = grp.rename(columns={'Descrição da conta':'Descricao da conta'})
    rows = []
...
def gerar_excel(grp, mc, mes, ano):
    ...
    for pac in pacotes:
        ...
        nc=11; ws2.merge_cells('A1:{}'.format(get_column_letter(nc))+'2')
    ...
    wm.row_dimensions[5].height=10; wm.merge_cells('B6:C6'); wm['B6']='  VISOES GERAIS'
    ...
    wm.row_dimensions[7].height=24; wm.row_dimensions[8].height=10
    wm.merge_cells('B9:C9'); wm['B9']='  PACOTES'
    wm['B9'].font=Font(name=FN,bold=True,size=9,color='FFFFFF'); wm['B9'].fill=fl(CP)
    wm['B9'].alignment=Alignment(horizontal='left',vertical='center'); wm.row_dimensions[9].height=18
...
if orc_file and pac_file:
    ...
    with st.spinner("Processando dados..."):
        try:
            ...
            # Normalizar nome coluna descricao
            if 'Descrição da conta' in merged.columns:
    ...
    with col1:
        ...
        st.download_button("⬇️ Baixar Relatorio Excel",data=xls,
            file_name="Acompanhamento_Despesas_{}_{}.xlsx".format(mes_sel,'20'+ano_ref),
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    with col2:
        st.caption("Excel com MENU de navegacao + RESUMO + {} abas de pacote, com status colorido.".format(len(df_res)))
